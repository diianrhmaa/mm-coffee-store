...
if (isset($_POST['submit'])) {
    ...
    if ($password == $cpassword && $username != 'username' && $email != 'email') {
        ...
        if ($result->num_rows > 0) {
            ...
            // Get the maximum admin_id from the table
            $sql = "SELECT MAX(admin_id) AS max_id FROM admin";
            $result = mysqli_query($koneksi, $sql);
            $row = mysqli_fetch_assoc($result);
            $max_id = $row['max_id'];

            // Set the new admin_id based on the maximum value
            $admin_id = $max_id + 1;

            $sql = "INSERT INTO admin (admin_id, username, password, email) VALUES (?, ?, ?, ?)";
            $stmt = mysqli_prepare($koneksi, $sql);
            mysqli_stmt_bind_param($stmt, "isss", $admin_id, $username, $password, $email);
                    ...
                    <div class="card-body p-0">
                        <div class="row">
                            <div class="col-lg-5 d-none d-lg-block">
                                <img src="../assets/img/reg1.jpg" style="width: 100%; height: 100%" />
                            </div>
                            <div class="col-lg-7">
                                <div class="p-5">
